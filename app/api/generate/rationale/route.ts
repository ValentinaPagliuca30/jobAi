...
import { assembleMatchRationalePrompt } from "@/lib/ai-prompts";
...
export async function POST(request: Request) {
  try {
    ...
    const prompt = assembleMatchRationalePrompt({
      profile,
    ...
    if (isAnthropicConfigured()) {
      try {
        const result = await generateWithClaude("match-rationale", prompt, {
          maxTokens: 256,
        });
        draft = result.text;
      } catch (err) {
        console.error(
          "[ai-client] match-rationale generation failed, falling back to stub",
          err,
        );
        draft = generateStub("match-rationale", prompt, stubMeta);
      }
    } else {
      draft = generateStub("match-rationale", prompt, stubMeta);
    }

    const updated = await setMatchRationaleDraft({
      clerkUserId: userId,
      applicationId: body.applicationId,
      draft,
    });
    ...
    return NextResponse.json({
      draft,
      generatedAt: updated.matchRationaleGeneratedAt,
    });
  } catch (error) {
    console.error("POST /api/generate/rationale failed", error);
    return NextResponse.json(
